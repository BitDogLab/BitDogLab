...
# Inicialize o buzzer fora da função
buzzer = PWM(Pin(21))

# Flag para a primeira identificação das coordenadas
first_fix = True
...
while True:
    ...
    if gps_module.any():
        ...
        altitude = gps.altitude if gps.altitude != None else 'N/A'  # Obtém a altitude

        oled.fill(0)
            
        if latitude and longitude:
            #oled.text('Satelites: ' + str(gps.satellites_in_use), 0, 0)
            oled.text('Lat: ' + str(latitude), 0, 20)
            oled.text('Lon: ' + str(longitude), 0, 30)
            oled.text('Alt: ' + str(altitude) + ' m', 0, 40)  # Exibe a altitude

            # Verifique se é a primeira vez que as coordenadas são identificadas
            if first_fix:
                awakening()  # Toca o alerta do buzzer
                first_fix = False  # Atualiza a flag para não tocar novamente
            ...
            if initial_lat is not None and initial_lon is not None:
                ...
                oled.text('Dist:{:.0f}m'.format(distance), 0, 0)

        else:
            oled.text("Aguardando GPS", 0, 0)
...
'''    if gps_module.any():
        ...
        if latitude and longitude:
            ...
            oled.text('Alt: ' + str(altitude) + ' m', 0, 50)  # Exibe a altitude
