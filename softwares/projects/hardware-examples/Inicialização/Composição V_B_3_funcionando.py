...
while True:
    ...
    while True:  # Loop externo para voltar após o botão do joystick ser pressionado
        if joystick_button.value():  # Continua enquanto o botão do joystick não é pressionado
            vrx_value = adc_vrx.read_u16()
            vry_value = adc_vry.read_u16()
            update_oled("APERRTE B SAIR")
            ...
            break  # Sai do modo joystick se o botão do joystick não está pressionado
    
    # Modo VU Meter
    while True:
        if button_b.value():  # Continua enquanto o botão B não é pressionado novamente
            ...
            break  # Sai do modo VU Meter se o botão B estiver pressionado
