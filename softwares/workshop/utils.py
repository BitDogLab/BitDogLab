...
matriz = neopixel.NeoPixel(Pin(PIN), NUM_LEDS)
...
def apaga():
    for y in range(5):
        for x in range(5):
            leds(x, y, 0, 0, 0)
    matriz.write()  # Forçar atualização dos LEDs
    time.sleep(0.1)  # Pequeno atraso para garantir a aplicação da atualização
    matriz.write()  # Segunda escrita para garantir que o buffer é atualizado
