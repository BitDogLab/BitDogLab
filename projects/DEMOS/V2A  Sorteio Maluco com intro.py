...
def draw_arrow_right():
    ...
    for row in range(5):
        for col in range(5):
            idx = row * 5 + col  # Calcula o índice do LED na matriz linear
            if arrow_pattern[row][col]:
                np[idx] = (10, 0, 0)  # LED em vermelho
            else:
                np[idx] = (0, 0, 0)  # LED apagado

    np.write()
    
def clear_matrix():
    np.fill((0, 0, 0))  # Define todos os LEDs como preto
    np.write()
...
def welcome_message():
    ...
    # Os botões estão configurados como Pin.IN com um Pin.PULL_UP,
    # o que significa que quando não são pressionados, eles têm um valor
    # lógico "alto" (ou True no contexto do código Python).
    # Quando você os pressiona, eles são puxados para o nível
    # lógico "baixo" ou False.
    # O lop continuará rodando enquanto ambos os botões não estiverem pressionados
    # (ou seja, ambos estão em estado alto/True). Se você pressionar um deles (ou ambos),
    # um ou ambos os valores serão False, quebrando a condição do and, e saindo do loop.
    # Portanto, pressionar qualquer botão (A ou B) vai interromper o loop e avançar o programa, mesmo com o and.
...
def display_numbers(user_num, random_num=None):
    ...
    oled.text("Seu numero: " + str(user_num), 10, 18)
    
    if random_num:
        oled.text("Sorteio: " + str(random_num), 10, 30)
        # Note que, ao invés de "limpar" as linhas 40 e 50, simplesmente não escrevemos nada nelas neste caso
    else:
        oled.text("Prescione:", 10, 30)
        oled.text("A para sortear", 10, 40)
        oled.text("B para mudar", 10, 50)
...
clear_matrix()  # Limpe a matriz depois de sair do loop acima
